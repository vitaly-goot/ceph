...
* **Run OSDs as root; log to the ZFS pool** via
  ``ReadWritePaths=/usr/local/akamai/logs`` on the ``ceph-osd@`` unit -
...
* **Drive load from a remote client** (here ``.68``) - A single client
