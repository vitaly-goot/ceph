...
and stays stable at any concurrency**. Crimson does not sustain replicated
...
omap path. One **clean 25-min soak** (5x4, no ``--memory`` cap) reached
**38k IOPS** (vs classic 183k) with a flat ~30-38k ceiling
and p99 up to ~1.15 s, but that clean window is not reliably reproducible.
**Crimson is not production-ready for replicated write workloads in this build,
and offers no throughput advantage to justify the wait.**
...
cluster can hold. **4 MiB sequential** topped out ~7.2 GiB/s from a single
client (parallelism-limited; more clients needed to characterize 4M).
...
* The single client is NOT the bottleneck. The client stays **83-92% idle**
...
* If testing continues: run thin OSDs (1 OSD/NVMe) **with NO ``--memory`` cap**.
  ...
  clients to characterize 4M bandwidth), and keep client concurrency at or below
  ~nj48 aggregate (the nj64 abort).
