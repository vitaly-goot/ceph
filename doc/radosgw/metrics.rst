...
* Ratio of noncurrent vs current expirations (high ratio indicates a versioning-heavy workload). The ``> 0`` clause on the denominator filters out buckets with no current expirations to avoid divide-by-zero:
