...
```
  ...
  /srv/exporter_scripts/slow_ops_osd_restarts.sh
  /srv/exporter_scripts/socket_stat.sh
```
...
```
  ...
  process_hist_ops=0                            # Inspect hist_ops
  process_inflight_ops=1                        # Inspect inflight_ops
  slow_ops_size_threshold=10                    # Trigger condition: number of slow ops
  slow_ops_duration_threshold=3                 # Trigger condition: slow ops duration (seconds)
  unwind_profiler_probes=0                      # Number of profiler probes to collect (0 disables profiler)
  profiler_run_delay=0                          # Delay before profiler run
  ...
  restart_osd=1                                 # Restart *primary* OSD for affected PG when slow_ops_size_threshold && slow_ops_duration_threshold confition met 
  session_max_duration=((2**32))                # Maximum duration for a debug session; at the end, logs from affected OSDs are retrieved from remote hosts to $debugfolder.
  session_end_enforced=0                        # Forcefully terminate the debug session and fetch all logs.
```

### Integration with Prometheus
When the restart_osd option is enabled, node restart events will be reported to Prometheus using a new metric, slow_ops_osd_restarts, which is a counter.
The reporting script is located at:
```
srv/exporter_scripts/slow_ops_osd_restarts.sh
```

### Socket Statistics Reporting
The socket statistics script reports ephemeral port usage to Prometheus using metric, socket_stat_ephemeral_port_usage, which is a gauge.
The reporting script is located at:
```
srv/exporter_scripts/socket_stat.sh
```

It should be added to the crontab to run every 5 minutes:
```
*/5 * * * * /srv/exporter_scripts/socket_stat.sh
```
