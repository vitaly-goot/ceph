...
int init_encrypter(CephContext *const cct, bool enable, const std::string &encrypt_key_file, uint64_t reload_interval)
{
  ...
  TheSecretEncrypter.reset(new RGWSecretEncrypterImpl(cct, enable, encrypt_key_file, reload_interval));
  return 0;
}

void reset_encrypter()
{
    TheSecretEncrypter.reset();
}
...
std::tuple<bool, uint32_t, bufferlist> RGWSecretEncrypterImpl::decrypt(uint32_t key_id, const bufferlist &secret, bufferptr&iv, const std::string &differentiator)
{
  ...
  if (key_id > 0 && !db_in_use->empty() && db_in_use->rbegin()->first < key_id) {
    ...
    db_in_use = curr_db; // Refresh the local reference
  }
  ...
  if (key_found == db_in_use->end()) {
    // Unknown key_id: Try reloading the key file, but be careful not to
    // reload it too frequently.
    time_t now = time(NULL);
    if (now > last_reload_for_unknown_key && (now - last_reload_for_unknown_key) >= (time_t)reload_interval) {
      last_reload_for_unknown_key = now;
      if (reload_keys(0) >= 0) {
        // Refresh db_in_use and suggested_key after successful reload
        db_in_use = curr_db;
        suggested_key = enabled ? get_key_to_use(*db_in_use.get()) : RGWEncryptKey{0, ""};
