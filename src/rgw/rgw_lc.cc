	...
	if (worker_should_stop(stop_at, once)) {
      ...
      if (now_ts >= next_mtime_update) {
        ...
        if (lr >= 0) {
          ...
          rgw::sal::LCEntry cur_entry;
          if (sal_lc->get_entry(this, null_yield, lc_oid, shard_id, cur_entry) >= 0) {
            cur_entry.mod_time = now_ts;
            cur_entry.instance = cct->_conf->name.to_str();
            ret = sal_lc->set_entry(this, null_yield, lc_oid, cur_entry);
			  ...
			  bool once = false)
{
  ...
  if (ret >= 0) {
    if (entry.status == lc_processing) {
      if (expired_session(entry.start_time, entry.mod_time)) {
  ...
  entry.mod_time = ceph_clock_now();
  entry.instance = cct->_conf->name.to_str();
  ret = sal_lc->set_entry(this, null_yield, obj_names[index], entry);
  if (ret < 0) {
